...
bootstrap dir bv config@S.Config{..} = do
  -- revision 
  cmd Shell ["./update-revision", ">", revision $ showMach m]
  -- make allx
  liftIO $ D.createDirectoryIfMissing False (dir </> "a1")
  e <- allAndCheckboot ("../boot" </> showMach m) "a1" bv config
  if e then do
    pure "a1"
    doCopy (showMach m) dir "a1" -- "a1/scheme.boot" "a1/petite.boot" "a1/scheme.h" "a1/equates.h"
    else do
    liftIO $ D.createDirectoryIfMissing False (dir </> "a2")
    e2 <- allAndCheckboot "a1" "a2" bv config
    if e2 then do
      pure "a2"
      doCopy (showMach m) dir "a2" -- "a2/scheme.boot" "a2/petite.boot" "a2/scheme.h" "a2/equates.h"
      else do
      liftIO $ D.createDirectoryIfMissing False (dir </> "a3")
      e3 <- allAndCheckboot "a2" "a3" bv config
      if e3 then do
        doCopy (showMach m) dir "a3" -- "a3/scheme.boot" "a3/petite.boot" "a3/scheme.h" "a3/equates.h"
        else do
