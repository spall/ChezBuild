...
bootTarget m' o d what C.Config{..} = do
  ...
  c <- semiConfigure m'
  withCmdOptions [Cwd (".." </> "xc-" ++ showMach m' </> "s")] $ xboot (".." </> "xc-" ++ showMach m' </> "s") ("../.." </> showMach m) what m o d c S.Config{..}
  -- cmd [Cwd $ ".." </> "xc-" ++ showMach m' </> "s"] ["make", "-f", "Mf-cross", "base=../../pb", "m=pb", "xm=ta6le"]

  withCmdOptions [Cwd (".." </> "xc-" ++ showMach m' </> "s")] $ S.Base.keepbootfiles (".." </> "xc-" ++ showMach m' </> "s") ("../boot/tmp" </> showMach m') c
...
bootquick m' config@C.Config{..} j = do
  ...
  withCmdOptions [Cwd "c"] $ C.Unix.build config j
...
build config@C.Config{..} j = do
  ...
  withCmdOptions [Cwd "s"] $ S.Unix.build "s" config
