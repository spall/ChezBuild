  ...
  case args of
    ...
    "--bootquick":m2:rest -> do
      ...
      case lookup mach machsBootquick of
        Just act -> do
          rattleRun (quickOptions j) $ act (showMach $ m config) mach config j
        Nothing -> do
    ...
    "--build":rest -> case lookup (m config) machsBuild of
                        Just act -> do
                          let j = case rest of
                                "--j=":j:_ -> read j
                                _ -> 0
                          rattleRun (buildOptions j) $ act (showMach $ m config) config j
                        Nothing -> do
    ...
    "--install":v:rest -> case lookup (m config) machsInstall of
                            Just act -> do
                              ...
                              let dir = "boot-final" </> (showMach $ m config) in
                                rattleRun (installOptions j) $ act (showMach $ m config) v dir config
                            Nothing -> do
